            ...
            db.getProducts(function (prods) {
                ...
                switch (req.session.typeUser) {
                    case 1:
                        ...
                        res.render('product', { product: tempB, iva: iva, typeOne: req.session.typeUser, stk : stk});
                        break;
                    case 2:
                        iva = tempB.Prices.PVP2 * tempB.IVA * 0.01;
                        res.render('product', { product: tempB, iva: iva, typeTwo: req.session.typeUser, stk : stk});
                        break;
                    case 3:
                        iva = tempB.Prices.PVP3 * tempB.IVA * 0.01;
                        res.render('product', { product: tempB, iva: iva, typeThree: req.session.typeUser, stk: stk});
                        break;
                    case 4:
                        iva = tempB.Prices.PVP4 * tempB.IVA * 0.01;
                        res.render('product', { product: tempB, iva: iva, typeFour: req.session.typeUser, stk: stk});
                        break;
                    case 5:
                        iva = tempB.Prices.PVP5 * tempB.IVA * 0.01;
                        res.render('product', { product: tempB, iva: iva, typeFive: req.session.typeUser, stk : stk});
                        break;
                    case 6:
                        iva = tempB.Prices.PVP6 * tempB.IVA * 0.01;
                        res.render('product', { product: tempB, iva: iva, typeSix: req.session.typeUser, stk : stk});
                        break;
                    default:
                        iva = tempB.Prices.PVP1 * tempB.IVA * 0.01;
