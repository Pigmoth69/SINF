...
namespace WebStoreAPI.Lib_Primavera
{
    public class PriIntegration
    {
        ...
        public static List<Model.DocVenda> Encomenda_GetClientsOrdersByPage(string client, int page, int numperpage)
        {
            ...
            string st = "WITH Orders AS (SELECT ROW_NUMBER() OVER (ORDER BY id) AS RowNum,id,Entidade,Data,NumDoc,TotalMerc,Serie FROM CabecDoc where TipoDoc='ECL' and Entidade='" + client + "') SELECT * FROM Orders WHERE  RowNum >= (" + page + "- 1) * "+numperpage+"  AND RowNum < ("+page+") * "+numperpage+" ORDER BY Data DESC;";
            objListCab = PriEngine.Engine.Consulta(st);
